"""Browser session exchange for PAPERLESS_API_TOKEN (hashed, short-lived)."""
...
from paperless_agent.local_security import COOKIE_NAME, get_api_token, token_matches

# Default 24h — long enough for a workday; override with PAPERLESS_SESSION_TTL_SECONDS.
...
def session_ttl_seconds() -> int:
    raw = os.getenv("PAPERLESS_SESSION_TTL_SECONDS", "").strip()
    if not raw:
        return _DEFAULT_TTL
    try:
        value = int(raw)
...
def exchange_api_token(api_token: str | None) -> str | None:
    """Validate PAPERLESS_API_TOKEN and return a new raw session id, or None."""
    expected = get_api_token()
    if not expected or not token_matches(api_token, expected):
        return None
...
def attach_session_cookie(
    ...
    *,
    ...
    """Set the HttpOnly pa_session cookie (random id — never the API secret)."""
    response.set_cookie(
        key=COOKIE_NAME,
