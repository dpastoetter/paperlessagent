...
def is_trusted_proxy(host: str | None) -> bool:
    """True only for an explicit PAPERLESS_TRUSTED_PROXIES IP (never hostnames)."""
...
def configured_bind_host() -> str:
    """Bind host from PAPERLESS_HOST, defaulting to loopback."""
    raw = (os.getenv("PAPERLESS_HOST") or "").strip()
    return raw or DEFAULT_BIND_HOST


def configured_bind_port() -> int:
    """Bind port from PAPERLESS_PORT, defaulting to 8080."""
    raw = (os.getenv("PAPERLESS_PORT") or "").strip()
    try:
        port = int(raw)
    except ValueError:
...
def sync_configured_bind(host: str, port: int | None = None) -> None:
    """Keep PAPERLESS_HOST / PAPERLESS_PORT aligned with the process bind."""
    os.environ["PAPERLESS_HOST"] = (host or "").strip() or DEFAULT_BIND_HOST
    if port is not None:
        os.environ["PAPERLESS_PORT"] = str(port)
...
def effective_bind_host() -> str:
    """Address the security policy must check: uvicorn's bind, else PAPERLESS_HOST.

    ``uvicorn app.main:app --host 0.0.0.0`` can diverge from PAPERLESS_HOST.
...
def forwarded_client_host(
    *,
    ...
    PAPERLESS_TRUSTED_PROXIES. The chain is walked right-to-left (nearest
...
def remote_auth_must_be_https(
    *,
    ...
    PAPERLESS_TRUSTED_PROXIES is the reverse-proxy hop, not the user — those
